...
def load_embedding_dict(embedding_path, embedding_size):
  ...
  logging.info('Loading word embeddings:' + str(current_time))
  with open(embedding_path, encoding='utf-8') as f:  #add encoding
  ...
  current_time = time.asctime(time.localtime(time.time()))
  logging.info('Done loading word embeddings:' + str(current_time))
...
def get_example(generator, glove_dict, batch_size, answer_num,
  ...
  while True:
    ...
    for i in range(bsz):
      ...
      if len(left_seq) > seq_length: #取离mention最近的seq_length个词
...
class TypeDataset(object):
  ...
  def _get_sentence(self, epoch, forever, eval_data, mode):
  ...
  def get_batch(self, batch_size=128, epoch=5, forever=False, eval_data=False, simple_mention=True, mode='dev'):
    return get_example(self._get_sentence(epoch, forever=forever, eval_data=eval_data, mode=mode), self.glove_dict,
