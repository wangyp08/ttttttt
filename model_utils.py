...
def sort_batch_by_length(tensor: torch.autograd.Variable, sequence_lengths: torch.autograd.Variable):
  ...
  sorted_tensor = tensor.index_select(0, permutation_index)  #按行索引
...
class SelfAttentiveSum(nn.Module):
  ...
  def __init__(self, output_dim, hidden_dim): #LSTM:200dim, 100dim
  ...
  def forward(self, input_embed):
    ...
    weighted_values = torch.sum(weighted_keys * input_embed, 1)  # batch_size, seq_length, embed_dim  按行求和
