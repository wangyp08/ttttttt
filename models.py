...
class Model(nn.Module):
  ...
  def sorted_rnn(self, sequences, sequence_lengths, rnn):
    ...
    packed_sequence_input = pack_padded_sequence(sorted_inputs,
                                                 ...
                                                 batch_first=True)   #压缩：去除多余的PAD的影响
    packed_sequence_output, _ = rnn(packed_sequence_input, None)
    unpacked_sequence_tensor, _ = pad_packed_sequence(packed_sequence_output, batch_first=True)  #解压
    return unpacked_sequence_tensor.index_select(0, restoration_indices)  #返回复原后的经过LSTM的embed
  ...
  def define_loss(self, logits, targets, data_type):
    ...
    if not data_type == 'kb':
      if final_cutoff:
        ...
        logit_masked = logits[:, fine_cutoff:]  # all 'open'
  ...
  def forward(self, feed_dict, data_type):
    if self.lstm_type == 'two':
      ...
      context_rep = torch.cat((left_outputs, right_outputs), 1)  #横向拼接
      context_rep, _ = self.attentive_sum(context_rep)
    elif self.lstm_type == 'single':
      token_mask_embed = self.token_mask(feed_dict['token_bio'].view(-1, 4))  #(batch_size, 4) -> (batch_size, 50)
    ...
    output = torch.cat((context_rep, mention_embed), 1)   #[batch_size, embed_dim]
    logits = self.decoder(output, data_type)  #[batch_size, answer_num]
